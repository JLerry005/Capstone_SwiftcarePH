<?php
    ...
    // Get Rejected
    $getRejected = $conn->query("SELECT * FROM rejectedreservations WHERE listing_id = $listingID;") or die($conn->error);
    if (mysqli_num_rows($getRejected)==0) {
        $rejected = '<p class="col-span-12 text-center font-bold text-blue-300 self-center "><i class="bi bi-emoji-frown-fill"></i> There are NO Rejected Reservations..</p> ';
    }
    else{
        while ($row = mysqli_fetch_assoc($getRejected)) {
            ...
            $rejected .='
                <a href="rejected-details?bookingID='.$bookingID.'" class=" col-span-4 bg-red-600 rounded-lg text-gray-400 hover:scale-105 hover:drop-shadow-md hover:cursor-pointer transition duration-100 ease-out"> 
    ...
    if ($getRejected) {
       // Get Expired
        $getExpired = $conn->query("SELECT * FROM expiredreservations WHERE listing_id = $listingID;") or die($conn->error);
        if (mysqli_num_rows($getExpired)==0) {
            $expired = '<p class="col-span-12 text-center font-bold text-blue-300 self-center "><i class="bi bi-emoji-frown-fill"></i> There are NO Expired Reservations..</p> ';
        }
        else{
