<?php
    ...
    if (mysqli_num_rows($result)) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<a href="hospital-overview?listingID='.$row["listing_id"].'" class="p-3 flex flex-1 hover:bg-gray-200 font-bold text-xs lg:text-base uppercase">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd" />
            </svg>
            &ensp;'.$row["hospital_name"].'&ensp;<p class="text-xs lg:text-sm text-slate-300 font-light">
