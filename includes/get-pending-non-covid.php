<?php
    ...
    $output = '';
    $result = $conn->query("SELECT * FROM (SELECT * FROM userbooking WHERE patientConcern = 'Non-Covid') userbooking WHERE listing_id = $listingID;") or die($conn->error);

    if (mysqli_num_rows($result)==0) {
        $output = '<p class="col-span-12 text-center font-bold text-blue-300 self-center "><i class="bi bi-emoji-frown-fill"></i> There are 0 Non-Covid reservation..</p> ';
    }
    else{
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $output .='
                <a href="pending-booking-details?bookingID='.$bookingID.'" class="col-span-4 bg-gray-900 rounded-lg text-gray-400 hover:scale-105 hover:shadow-md hover:shadow-gray-900 hover:cursor-pointer transition duration-700 ease-out"> 
